...
var BlockChain = /** @class */ (function () {
  ...
  BlockChain.prototype.deriveTxHash = function (tx) {
    var derivedTx = __assign(__assign({}, tx), {
  ...
  BlockChain.prototype.deriveBlockHash = function (block) {
    var derivedBlock = __assign(__assign({}, block), {
      merkleRoot: lodash_1["default"].head(
        this.deriveMerkleRoot(
          lodash_1["default"].map(block.transactions, function (tx) {
            return tx.hash;
          })
        )
      )
    });
    return __assign(__assign({}, derivedBlock), {
      hash: crypto_js_1["default"]
        .SHA256(
          JSON.stringify({
  ...
  BlockChain.prototype.deriveMerkleRoot = function (hashes) {
    ...
    if (lodash_1["default"].isNull(hashes)) return [""];
    if (lodash_1["default"].isEqual(hashes.length, 0)) return [""];
    if (lodash_1["default"].isEqual(hashes.length, 1)) return hashes;
    var root = [hashes];
  ...
  BlockChain.prototype.addPendingTransaction = function (tx) {
    ...
    return lodash_1["default"].filter(this.transactionPool, function (tx) {
      return (
        lodash_1["default"].isEqual(tx.hash, hashedCoinBaseTransaction.hash) ||
  ...
  BlockChain.prototype.prepareBlock = function (difficulty) {
    var block = {
      ...
      merkleRoot: "0",
      previousHash: lodash_1["default"].last(
        lodash_1["default"].map(this.chain, function (b) {
          return b.hash;
        })
      ),
      hash: "",
  ...
  BlockChain.prototype.proofOfTransaction = function (b) {
    log(
      "Checking transaction integrity for block ==> " +
        lodash_1["default"].get(b, "hash")
    );
    return lodash_1["default"].isEqual(
      lodash_1["default"].head(
        this.deriveMerkleRoot(
          lodash_1["default"].map(b.transactions, function (tx) {
            return tx.hash;
          })
        )
      ),
  ...
  BlockChain.prototype.mineBlock = function (difficulty) {
    ...
    while (
      ...
      log(
        "Iteration ==> " + iteration + " Derived hash ==> " + mineableBlock.hash
      );
    }
  ...
  BlockChain.prototype.chainIsValid = function () {
    for (var i = 1; i < this.chain.length; i++) {
      ...
      log(
        "Checking validity for block ==> " +
          lodash_1["default"].get(currentBlock, "hash")
      );
      ...
      if (
        !lodash_1["default"].isEqual(
          currentBlock,
          this.deriveBlockHash(currentBlock)
        )
      )
        return false;
