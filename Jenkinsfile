 ...
 stages {
  ...
  stage("Create build details file & SCM tag") {
   steps {
    ...
    bat("if not exist jenkins (mkdir jenkins) && move build-${JOB_NAME}-${BUILD_NUMBER}-${BUILD_ID} ./jenkins/build-${JOB_NAME}-${BUILD_NUMBER}-${BUILD_ID} && del build-${JOB_NAME}-${BUILD_NUMBER}-${BUILD_ID}")
